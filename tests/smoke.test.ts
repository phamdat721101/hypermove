...
describe('hypermove-app · S1 scaffold smoke', () => {
  ...
  it('tailwind config encodes HyperMove design tokens (single source)', () => {
    ...
    expect(cfg).toContain('#030213'); // hm-primary
    expect(cfg).toContain('#635bff'); // hm-purple
    expect(cfg).toContain('#00d4ff'); // hm-blue
    expect(cfg).toContain('#f7f9fc'); // hm-bg
  });
  ...
  it('.well-known/webmcp.json is independent (no GOAT framing)', () => {
    ...
    expect(manifest.name).toBe('hypermove.duckdns.org');
    expect(manifest.version).toBe('4.0');
    expect(JSON.stringify(manifest)).not.toMatch(/GOAT.*Builder.*Grant/i);
  });

  it('homepage advertises the per-call model (5 free calls, no subscription)', () => {
    const page = read('src/app/page.tsx');
    expect(page).toMatch(/per call/i);
    expect(page).toMatch(/5 free/i);
    // The old BTC-subscription pricing card + /pricing link are gone.
    expect(page).not.toMatch(/\/pricing/);
    expect(page).not.toMatch(/\$5\/month/i);
  });
